...
def update_subscription(user_id: str, plan_type: str, amount_paid: float) -> Dict[str, Any]:
    """Activate a paid plan and refresh credit score in a single DB transaction."""
    clean_uid = user_id.strip("@").lower().strip()
    if plan_type not in ("landowner_listing", "corporate_access"):
        raise ValueError("plan_type must be landowner_listing or corporate_access")

    conn = get_db_connection()
    c = conn.cursor()
    now = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")

    c.execute("SELECT * FROM users WHERE user_id = ?", (clean_uid,))
    row = c.fetchone()
    if not row:
        conn.close()
    ...
    c_res = calculate_user_credit_score(
        ...
        stated_budget_inr=float(user.get("budget_inr") or 0),
    ...
    c.execute(
        """
        UPDATE users
        SET subscription_tier = ?, credit_score = ?, credit_tier = ?, credit_factors_json = ?
        WHERE user_id = ?
        """,
        (
            plan_type,
    ...
    c.execute(
        ...
        VALUES (?, ?, ?, 'active', ?)
        """,
        (clean_uid, plan_type, float(amount_paid), now),
    )
    conn.commit()
    conn.close()

    updated = get_user_by_id(clean_uid)
    if not updated:
        raise ValueError(f"User @{clean_uid} missing after subscription update.")
    return updated
